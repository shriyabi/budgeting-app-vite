...
//"https://budgeting-app-vite.onrender.com"; 
...
export default function SpendingAnalyzer({ isOpen, onClose, currentItems = [], netIncome = 0 }) {
  ...
  }; //adding new bank statements per month
  ...
  const handleAnalyze = async () => {
    ...
    const validStatements = statements
      ...
      .map(rawText => {
        ...
        // Format them cleanly for the AI: "Date | Description | $Amount"
        return parsedTxns
          .map(t => `${t.date} | ${t.description} | $${t.amount.toFixed(2)}`)
          .join('\n');
      });

    console.log(validStatements); 
    //budget context to tweak budget vs no budget == general advice
    ...
    try {
      ...
      setAiResponse(data.analysis || data.message || "Analysis complete. Check console for raw output.");
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-[90vw] md:max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-emerald-500/20">
          ...
          {aiResponse ? (
            <div className="animate-in slide-in-from-bottom-4">
              <div className="flex justify-between items-end mb-4">
                ...
                <button onClick={() => setAiResponse(null)} className="text-xs text-gray-400 hover:text-emerald-500 underline font-mono">← New Analysis</button>
              </div>
              <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 text-sm leading-relaxed whitespace-pre-wrap shadow-inner font-mono">
                {aiResponse}
              </div>
            </div>
          ) : (
            ...
            <div className="space-y-6 animate-in fade-in">
              ...
              <div>
                <label className="block text-xs font-bold uppercase font-mono text-gray-500 mb-2">
                  1. List your Financial Goals
                </label>
                <textarea 
                  placeholder="e.g., I want to save $5,000 for a trip to Japan, but I feel like I eat out too much..."
                  className="w-full p-4 font-semibold font-sans rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm min-h-[80px]"
                  value={goals}
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  <label className="block text-xs font-bold uppercase font-mono text-gray-500">
                    2. Add Bank Statements
                  </label>
                  <span className="text-[9px] text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30 px-2 py-1 rounded font-bold uppercase">
                    {statements.length} Added
                  </span>
                </div>
                ...
                <div className="space-y-3">
                  {statements.map((stmt, index) => (
                    <div key={index} className="relative group">
                      ...
                      {statements.length > 1 && (
                        <button 
                          onClick={() => removeStatementField(index)}
                          className="absolute top-3 right-3 w-6 h-6 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-rose-500 hover:text-white"
                ...
                <button 
                  onClick={addStatementField}
                  className="mt-3 flex items-center gap-2 text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 transition-colors"
                >
        ...
        {/* footer == close/open */}
        {!aiResponse && (
          <div className="p-4 bg-white dark:bg-gray-800 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-700 flex-shrink-0">
            <button 
              onClick={onClose} 
              className="px-4 py-2 text-gray-500 font-bold hover:text-red-500 font-mono text-sm"
            >
              CANCEL
            </button>
            <button 
              onClick={handleAnalyze} 
              disabled={loading || statements.every(s => s.trim() === '')}
              className="px-6 py-2 bg-emerald-600 text-white rounded-xl font-bold shadow-lg hover:bg-emerald-700 transition-transform active:scale-95 disabled:opacity-50 font-mono text-sm flex items-center gap-2"
            >
              {loading ? (
